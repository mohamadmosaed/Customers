...
<div class="container mt-5">
    <div class="card p-4">
        ...
        <div class="d-flex justify-content-between mb-3">
            <a href="{{ route('agents.create') }}" class="btn btn-success btn-custom">➕ إضافة مندوب جديد</a>
        </div>
            ...
            <table class="table table-hover table-bordered bg-white">
                ...
                <tbody>
                    @foreach ($agents as $agent)
                        <tr>
                            <td>{{ $agent->name }}</td>
                            <td>{{ $agent->Work_area ?? 'غير محدد' }}</td>
                            <td>{{ $agent->start_date }}</td>
                            <td>{{ $agent->end_date }}</td>
                            <td>
                                <a href="{{ route('agents.show', $agent->id) }}" class="btn btn-info btn-sm btn-custom">👁 عرض</a>
                                <a href="{{ route('agents.edit', $agent->id) }}" class="btn btn-primary btn-sm btn-custom">✏ تعديل</a>
